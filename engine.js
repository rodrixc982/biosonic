		...
		this.LoadArrayBuffer = function ( e ) {
			...
			if ( q.is_ready )
			{
					...
					new PKSimpleModal ({
						...
						body    : '<p>¿Anexar archivo a una pista existente?</p>',
						setup   : function( q ) {
...
//			audio_el.playbackRate = 0.5;
		...
		wavesurfer.on ('pausa', function() {
			app.fireEvent ('DidStopPlay');
		});
		wavesurfer.on ('play', function() {
			app.fireEvent ('DidPlay');
		});
		wavesurfer.on ('buscar', function ( where, stamp ) {
...
//		});
		...
		app.listenFor ('RequestActionPaste', function () {
			...
			// agregar una región donde ocurrió el pegado
			wavesurfer.regions.clear();
			wavesurfer.regions.add({
				start:dims[0],
				end:dims[1],
		...
		app.listenFor ('RequestActionRecordToggle', function () {
			if (!q.is_ready) {
				// si no está listo, ¡abre la nueva opción de grabación!
			...
			if (app.rec.isActive ()) {
				...
				// saltando los sonidos del teclado
				if (_sk) return ;
		...
		app.listenFor ('RequestActionRecordStart', function () {
			...
			// --- el desplazamiento final es la duración completa de la canción...
            // si tenemos un área seleccionada - marcar esa como el final
			var region = wavesurfer.regions.list[0];
			if (region)
