...
//				var freq = (rel_x * (total_freq) + 0) >> 0; // + 16 (min freq)
	...
	var ParagraphicModal = function ( app, custom_presets ) {
		...
		var x = new PKAudioFXModal ({
			...
			presets:[
					{name:'Teléfono viejo',val:'1,highpass,0,5800,5.8,1,lowpass,0,7060,5'}
			],
	...
	function _group_rhythm ( data, diff_arr ) {
			...
			for (var i = 0; i < data.length - 1; ++i) // #### do not litter the last data
			{
				...
				for (var j = i + 1; j < 1000; ++j) {
					...
					// si existe, busque inmediatamente el siguiente.
			...
			// agarrar sólo los grandes picos.
	...
	var TempoToolsModal = function ( app ) {
		...
		var TempoMetro = function ( app, modal ) {
			...
			var audioContext = null; // nuevo AudioContext();
			var osc = null;
			var amp = null;
			var ready = false;
			var volume = 0.5;
		...
		var TempoTap = function ( app, modal ) {
			...
			function _make_ui ( q ) {
				...
				// Botón Estimar tempo para el área seleccionada
...
//						ctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
		...
		var TempoEstimation = function ( app, modal ) {
			...
			q.Est = function ( selection ) {
				...
				var _pass = function ( rendered_buffer, offset, duration ) {
                    ...
                    if (!ret) {
                    	console.log ("sucedió algo raro, error 244");
                    	return ;
                    }
			...
			function _make_ui ( q ) {
				...
				'<label class="pk_lblmp3" for="tt5">Estimación solo para selección</label></div>' +
				'<div class="pk_row">' + 
				'<a class="pk_modal_a_bottom" style="margin:0;float:left">Estimar</a>'+
				'</div>';
		...
		var x = new PKAudioFXModal ({
			id: filter_id,
			title: 'Herramientas de tempo y ritmo',
	...
	var RecModal = function ( app ) {
		...
		var x = new PKAudioFXModal ({
			...
			body: '<div class="pk_rec" style="user-select:none">' +
				...
				'</div>' +
				'<div class="pk_row">' +
				  '<a class="pk_tbsa pk_inact" style="text-align: center;">EMPEZAR A GRABAR</a>' +
				  '<a class="pk_tbsa pk_inact" style="margin-left: 24px; text-align: center;">PAUSE</a>' +
				'</div>' +
				'<div class="pk_row">' +
					'<a class="pk_tbsa" style="float:left;display:none;text-align:center;box-shadow:0 0 7px #3a6b79 inset;">GRABACIÓN ABIERTA</a>' +
					'<a class="pk_tbsa" style="float:left;display:none;margin-left: 24px; text-align: center;">AÑADIR A EXISTENTE</a>' +
				'</div>' +
				'</div>',
...
//			}],
	...
	//						freqctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
					...
					navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(function( _stream ) {
						...
						alert("¡No se encontraron permisos de micrófono!");
					});
					...
					btn_start.onclick = function () {
						...
						// comprobar si existe grabación - pedir confirmación
						if (has_recorded) {
							if (!window.confirm("¿Está seguro? Esto descartará la grabación actual."))
							{
								return ;
							}
						}
						...
						if (is_active) {
							...
							btn_start.innerText = 'EMPEZAR A GRABAR';
						...
						navigator.mediaDevices.getUserMedia({ audio: audio_val }).then(function( stream ) {
			            	...
			            	btn_start.innerText = 'FINALIZAR LA GRABACIÓN';
