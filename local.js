(function ( w, d, PKAE ) {
	...
	function SaveLocal ( app ) {
		...
		this.SaveSession = function ( buffer, id, name ) {
			...
			var trans = db.transaction(['Sesiones'], 'LeerEscribir');
			var addReq = trans.objectStore('Sesiones').add(ob);
		...
		this.GetSession = function ( id, callback ) {
			var trans = db.transaction(['Sesiones'], 'Solo lectura');
			//hard coded id
			var req = trans.objectStore('Sesiones').get(id);
		...
		this.DelSession = function ( id, callback ) {
			...
			var req = trans.objectStore('Sesiones').delete (id);
			req.onsuccess = function (e) {
		...
		this.ListSessions = function ( callback ) {
			var trans = db.transaction(['Sesiones'], 'Solo lectura');
			var object_store = trans.objectStore('Sesiones');
			var req = object_store.openCursor();
			var ret = [];

			req.onerror = function(event) {
			   console.err("Error al obtener datos");
			};
