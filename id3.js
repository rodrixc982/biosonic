(function ( w, d, PKAE ) {
    ...
    ID3v2.frames = {
        // v2.2
        "BUF" : "Tamaño de búfer recomendado",
        "CNT" : "Play de Contador",
        "COM" : "Comentarios",
        "CRA" : "Encriptación de audio",
        "CRM" : "Metaframe encriptado",
        "ETC" : "Códigos de temporización de eventos",
        "EQU" : "Igualación",
        "GEO" : "Objeto encapsulado general",
        "IPL" : "Lista de personas involucradas",
        "LNK" : "Información enlazada",
        "MCI" : "Identificador de CD de música",
        "MLL" : "Tabla de búsqueda de ubicación MPEG",
        "PIC" : "Imagen adjunta",
        "POP" : "Popularímetro",
        "REV" : "Reverberación",
        "RVA" : "Ajuste de volumen relativo",
        "SLT" : "Letra/texto sincronizado",
        "STC" : "Códigos de tempo sincronizados",
        "TAL" : "Título del álbum/película/programa",
        "TBP" : "BPM (pulsaciones por minuto)",
        "TCM" : "Compositor",
        "TCO" : "Content type",
        ...
        "WXXX" : "Marco de enlace URL definido por el usuario"
    };

    var pictureType = [
        "'icono de archivo' de 32x32 píxeles (solo PNG)",
        "Otro icono de archivo",
        "Cubierta (frontal)",
        "Portada (trasera)",
        "Página del folleto",
        "Medios (por ejemplo, el lado de la etiqueta del CD)",
        "Artista principal/intérprete principal/solista",
        "Artista/intérprete",
        "Conductor",
        "audio/Nombre de etiqueta",
        "Compositor",
        "Letrista/escritor de textos",
        "Ubicación de grabación",
        "Durante la grabación",
        "Durante la actuación",
        "Captura de pantalla de película/video",
        "Un pez de colores brillantes",
        "Ilustración",
        "Logotipo de banda/audio",
        "Logotipo del editor/estudio"
    ];
    ...
    var readFrames = function (offset, end, data, id3header, tags) {
            ...
            while( offset < end ) {
                ...
                // si el último fotograma GTFO
                if( frameID == "" ) { break; }

                // avanzar los datos desplazados a los datos del siguiente cuadro
                offset += frameHeaderSize + frameSize;
                // omitir etiquetas no deseadas
                if( tags.indexOf( frameID ) < 0 ) { continue; }

                // leer mensajes de marco y formatear banderas
                if( major > 2 )
                {
                ...
                // los primeros 4 bytes son el tamaño real de los datos
                // (después de la sincronización y el cifrado)
                ...
                // encontrar la función de análisis de cuadros
    ...
    ID3v2.readFrameData['PCNT'] = function readCounterFrame(offset, length, data) {
        // FIXME: implementar el resto de la especificación
    ...
    ID3v2.ReadTags = function ( arraybuffer ) {
        ...
        // crear accesos directos para los datos más comunes
    ...
    ID4.loadData = function(arraybuffer, callback) {
        var data = new DataView ( arraybuffer );
        // cargar el encabezado del primer bloque
        loadAtom(data, 0, data.byteLength, callback);
    };

    /**
    * Asegúrese de que los bytes [offset, offset+7] (el encabezado del bloque) estén
      * ya cargado antes de llamar a esta función.
     */
