...
const FFT = function(bufferSize, sampleRate, windowFunc, alpha) {
    ...
    this.calculateSpectrum = function(buffer) {
        //Variables de alcance local para acelerar
        ...
        if (Math.pow(2, k) !== bufferSize) {
            throw 'Tamaño de búfer no válido; debe ser una potencia de 2.';
        }
        if (bufferSize !== buffer.length) {
            throw 'El búfer proporcionado no tiene el mismo tamaño que la FFT definida. FFT Size: ' +
                bufferSize +
                ' Buffer Size: ' +
                buffer.length;
        }
...
var MultiCanvas = function (_Drawer) {
    ...
    function MultiCanvas(container, params) {
        ...
        /**
         * Si se representa o no la onda progresiva. If the `waveColor`
