(function ( w, d, PKAE ) {
	...
	// 
	// CLASE DE UI PRINCIPAL
	var PKUI = function( app ) {
		var q = this;

		this.el = app.el;

		// si es móvil, agregue la clase adecuada
		this.el.className += ' pk_app' + (app.isMobile ? ' pk_mob' : '');
		
		// contener referencias a las funciones del evento
		this.fireEvent = app.fireEvent;
		this.listenFor = app.listenFor;

		// realizar un seguimiento del elemento activo de la interfaz de usuario
		...
		this.KeyHandler = new app._deps.keyhandler ( this ); // inicializando el controlador de claves
		this.TopHeader  = new _makeUITopHeader ( _topbarConfig ( app ), this ); // menú superior
		this.Toolbar    = new _makeUIToolbar ( this ); // barra de herramientas principal y controles
	...
	//Lista de configuración de la barra superior
	function _topbarConfig ( app, ui ) {
		return [
				...
				children : [
					{
						...
						action: function () {
								new PKSimpleModal({
								  ...
								  body:'<div class="pk_row"><label for="k0">Nombre del archivo</label>' + 
									'<input style="min-width:250px" placeholder="mp3 filename" value="BIOSONIC-IIAP.mp3" ' +
									'class="pk_txt" type="text" id="k0" /></div>'+

									'<div class="pk_row" id="frmtex" style="padding-bottom:4px"><label style="display:inline">Formato</label>'+
									...
									'<label for="k4">Exportar archivo completo</label>'+
									'<input type="radio" class="pk_check" id="k5" name="xport" value="sel">'+
									'<label class="pk_lblmp3" for="k5">Exportar solo selección</label></div>',
									  		...
									  		format && format.addEventListener('change', function(e){
												...
												for (var i = 0; i < inputs.length; ++i)
												{
													if (inputs[i].checked)
													{
														...
														{
															mp3conf.style.display = 'Bloquear';
															inputtxt.value = inputtxt.value.replace('.wav', '.mp3');
														}
														else
														{
															mp3conf.style.display = 'ninguno';
					...
					{
						name: 'Cargar desde la computadora',
						type: 'archivo',
						action: function ( e ) {
					...
					{
						name: 'Cargar archivo de muestra',
						action: function ( e ) {
					...
					{
						name: 'Cargar desde URL',
						action: function ( e ) {
								new PKSimpleModal({
								  title:'Cargar audio desde la URL remota',
								  ...
								  buttons:[
									{
										...
										callback: function( q ) {
											...
											{
												OneUp ('URL no válida introducida', 1100);
											}
											// -
										}
									}
					...
					{
						name: 'Guardar borrador localmente',
						clss: 'pk_inact',
						action: function ( e ) {
							...
							// modal que solicita: archivo completo, selección, búfer de copia
							new PKSimpleModal ({
								title : 'Guardar borrador local de...',
								...
								body:'<p>Por favor, elija la fuente..</p>' +
						...
						setup: function ( obj ) {
							...
							app.listenFor ('DidStoreDB', function ( obj, e ) {
									...
									new PKSimpleModal ({
										...
										body:'<p>¿Abrir en una ventana nueva?</p>' + txt,
										setup:function( q ) {
											app.fireEvent ('RequestPause');
											app.fireEvent( 'RequestRegionClear');
					...
					{
						name: 'Abrir borradores locales',
						action: function ( e ) {

							var datenow = new Date ();
							var time_ago = function ( arg ) {
							    var a = (datenow - arg) / 1E3 >> 0;
							    if (59 >= a) return datenow = 1 < a ? 's' : '', a + ' segundo' + datenow + ' hace';
							    if (60 <= a && 3599 >= a) return a = Math.floor(a / 60), a + ' minuto' + (1 < a ? 's' : '') + ' hace';
							    if (3600 <= a && 86399 >= a) return a = Math.floor(a / 3600), a + ' hora' + (1 < a ? 's' : '') + ' hace';
							    if (86400 <= a && 2592030 >= a) return a = Math.floor(a / 86400), a + ' día' + (1 < a ? 's' : '') + ' hace';
							    if (2592031 <= a) return a = Math.floor(a / 2592E3), a + ' mes' + (1 < a ? 's' : '') + ' hace';
							};
							var func = function ( fls ) {								
								fls.ListSessions(function( ret ) {
									...
									var msg = '';
									if (ret.length === 0) {
										msg += 'No se han encontrado borradores..';
									}
									else
									{
							...
							app.listenFor ('LoadDraft', function ( name, append ) {
									app.fls.Init (function (err) {
											...
											app.fls.DelSession (name, function (name) {
												...
												if (el)
												{
													if ( el.parentNode.children.length === 1 ) {
														el.parentNode.innerHTML = 'No se han encontrado borradores...';
													}
										...
										if (append === 3)
										{
											...
											// generar contexto de audio aquí...
											var aud_cont = new (w.AudioContext || w.webkitAudioContext)();
								            if (aud_cont && aud_cont.state == 'suspendido') {
										...
										// --- preguntar si queremos hacer clic en el primero
										if (app.engine.is_ready && !append)
										{
											var mm = new PKSimpleModal ({
												title : '¿Abrir en Existente?',
												body  : '<div>¿Abrir en una ventana nueva o en la actual?</div>',
												buttons:[
													{
														title:'ABRIR',
														clss:'pk_modal_a_accpt',
			...
			},
			{
				...
				children:[
					{
						name: 'Deshacer <span class="pk_shrtct">Shft+Z</span>',
						clss: 'pk_inact',
						action: function () {
						...
						setup: function ( obj ) {
							app.listenFor ('DidStateChange', function ( undo_states, redo_states ) {
								...
								{
									obj.innerHTML = 'Deshacer <span class="pk_shrtct">Shft+Z</span>';
									obj.classList.add ('pk_inact');
								}
								else
								{
					...
					{
						name: 'Rehacer <span class="pk_shrtct">Shft+Y</span>',
						clss: 'pk_inact',
						action: function () {
						...
						setup: function ( obj ) {
							app.listenFor('DidStateChange', function ( undo_states, redo_states ) {
								...
								{
									obj.innerHTML = 'Rehacer <span class="pk_shrtct">Shft+Y</span>';
									obj.classList.add ('pk_inact');
								}
								else
								{
					...
					{
						name: 'Seleccionar todo <span class="pk_shrtct">Shft+A</span>',
						action: function () {
					...
					{
						name: 'Anular la selección de todo <span class="pk_shrtct">~</span>',
						action: function () {
					...
					{
						name   : 'Información del canal/Flip',
						action : function () {
			...
			},
			{
				...
				children:[
					...
					{
						name   : 'Ecualizador gráfico (20 bandas)',
						action:function () {
					...
					{
						name   : 'Acelerar / Reducir la velocidad',
						action:function () {
			...
			},
			{
				...
				children:[
					...
					{
						name:'Analizador de espectrograma',
						action: function ( obj ) {
...

					{
						name:'TENSO FLOW',
						action: function ( obj ) {
							app.fireEvent ('RequestShowFreqAn', 'tensoflow', [1]);
						},
						setup: function ( obj ) {
							app.listenFor ('DidToggleFreqAn', function ( url, val ) {
								if (url !== 'tensoflow') return ;
					...
					{
						name:'Centrar al cursor<span class="pk_shrtct">[Tab]</span>',
						action: function ( obj ) {
					...
					{
						name:'Restablecer zoom <span class="pk_shrtct">[0]</span>',
						action: function ( obj ) {
			...
			},
			{
				...
				children:[
					{
						name   : 'Versión Para Escritorio ',
						action : function () {
							if (window.location.href.indexOf('-cache') > 0) {

								function onUpdateReady ( e ) {
									if (confirm ('¿Desea actualizar la página para cargar la versión más reciente?'))
										window.location.reload();
								}
								function downLoading ( e ) {
									OneUp ('Descargando la versión más reciente', 1500);
								}
							...
							var message = 'Esto abrirá una nueva ventana para Descagar el exe'; // nicer text

							new PKSimpleModal ({
								title : '¿Abrir la versión sin conexión?',
						...
						setup: function ( obj ) {
							if (window.location.href.indexOf('-cache') > 0)
							{
								obj.innerHTML = 'Actualizar la versión sin conexión';
							}
						}
					},
					...
					{
						name   : 'Ver Mensaje de Bienvenida',
						action : function () {
					...
					{
						name   : 'Pagina Del IIAP',
						action : function () {
							window.open ('http://www.iiap.org.pe/web/presentacion_iiap.aspx');
						}
					}
				]
			}
	...
	function _makeUIMainView ( UI, app ) {
		...
		// cambia el mensaje temporal, es bastante feo #### TODO
		var ttmp = d.createElement('div');
		ttmp.className = 'pk_tmpMsg';
		ttmp.innerHTML = 'Arrastre y suelte un archivo de audio en esta ventana o haga clic en ' +
		'<a style="white-space:nowrap;border:1px solid;border-radius:23px;padding:5px 18px;font-size:0.94em;margin-left:5px; color: white !important;" '+
		'onclick="PKAudioEditor.engine.LoadSample()">aquí prueba un audio rana</a>';
		
		main_audio_view.appendChild( ttmp );

		var ttmp2 = d.createElement('div');
		ttmp2.className = 'pk_tmpMsg2';
		ttmp2.innerHTML = '<span>Espera...</span><div class="pk_mload"><div></div></div>' + 
	...
	function _makeUIToolbar (UI) {
		...
		btn_play.innerHTML = '<span>Play (Espacio)</span>';
		...
		btn_pause.innerHTML = '<span>Pause (Shift+Espacio)</span>';
		...
		btn_loop.innerHTML = '<span>Bucle de palanca (L)</span>';
		...
		btn_back_jump.innerHTML = '<span>Buscar (flecha izquierda)</span>';
		...
		btn_front_jump.innerHTML = '<span>Buscar (flecha derecha)</span>';
		...
		btn_back_total.innerHTML = '<span>Inicio de búsqueda (Shift + left arrow)</span>';
		...
		btn_front_total.innerHTML = '<span>Buscar el final (Shift + right arrow)</span>';
		...
		btn_rec.innerHTML = '<span>Registro (R)</span>';
		...
		setTimeout(function () {
			...
			main_context.addOption ('Seleccione Vista visible', function( e,x,i ) {
				UI.fireEvent ('RequestRegionSet');
			}, false );

			main_context.addOption ('Restablecer zoom', function( e ) {
				UI.fireEvent ('RequestZoomUI', 0);
			}, false );

			main_context.addOption ('Ajuste de volumen/ganancia', function( e ) {
				UI.fireEvent ('RequestFXUI_Gain');
			}, false );

			main_context.addOption ('Copiar', function( e ) {
				var region = PKAudioEditor.engine.wavesurfer.regions.list[0];
				if (!region) return ;

				UI.fireEvent( 'RequestActionCopy');
			}, false );
			main_context.addOption ('Pegar', function( e ) {
				if (!copable) return ;
				UI.fireEvent( 'RequestActionPaste');
			}, false );
			main_context.addOption ('Cortar', function( e ) {
				var region = PKAudioEditor.engine.wavesurfer.regions.list[0];
				if (!region) return ;

				UI.fireEvent( 'RequestActionCut');
			}, false );
			main_context.addOption ('Insertar silencio', function( e ) {
		...
		copy_btn.innerHTML = '<span>Copiar selección (Shift + C)</span>';
		...
		paste_btn.innerHTML = '<span>Pegar selección (Shift + V)</span>';
		...
		cut_btn.innerHTML = '<span>Selección de corte (Shift + X)</span>';
		...
		silence_btn.innerHTML = '<span>Insertar silencio ...(Shift + N)</span>';
		actions.appendChild ( silence_btn );
		
        var save_btn = d.createElement('button');
        save_btn.className = 'pk_btn icon-save pk_inact';
        save_btn.innerHTML = '<span>Guardar Selección</span>';
		...
		selection.innerHTML = '<div class="pk_sellist">' + 
			'<span class="pk_title">Selección:</span>' + 
			'<div><span class="title">Empezar:</span><span class="s_s pk_dat">-</span></div>' + 
			'<div><span class="title">Fin:</span><span class="s_e pk_dat">-</span></div>' + 
			'<div><span  class="title">Duracion:</span><span class="s_d pk_dat">-</span></div>' +
		'</div>';
